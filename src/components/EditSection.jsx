...
function EditSection({ section, data, setter, handlers }) {
  ...
  const properties =
    section === 1 ? ['jobTitle', 'companyName', 'workingDate'] : ['degree', 'institutionName', 'graduationYear'];
          ...
          <div key={item.id} className='entry'>
            <div className='entry-title'>
              ...
              <div className='entry-buttons'>
                ...
                {entryExpanded === item.id ? (
                  <button onClick={() => setEntryExpanded(-1)}>
                    <img src={arrowLeft} alt='Arrow Left' />
                  </button>
                ) : (
                  <button onClick={() => setEntryExpanded(item.id)}>
                  ...
                  return (
                    <div key={achievement.id} className={entryExpanded === item.id ? 'entry-shown' : 'entry-hidden'}>
                      <div className='achievement-entry'>
                        ...
                        />
                        <button
                          onClick={() => handlers.handleAchievementsDeletion(item.id, achievement.id)}
