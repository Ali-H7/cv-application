    ...
    <div className='edit'>
      <div className='sections'>
        {editSections.map((sectionLabel, index) => (
          <button
            className={selectedSection === index ? 'selected' : 'not-selected'}
            onClick={() => {
      ...
      {selectedSection === 0 && (
        <EditProfileSection profileData={data.profile} handleProfileEdits={handleProfileEdits} />
      )}
      {selectedSection === 1 && (
        <EditSection
          section={selectedSection}
          data={data.workExperience}
          setter={setters.setWorkExperience}
          handlers={handlers}
        />
      )}
      {selectedSection === 2 && (
